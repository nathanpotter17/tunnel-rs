...
pub struct CryptoSession {
    ...
    /// Highest nonce we've seen for replay protection
    max_recv_nonce: AtomicU64,
...
impl CryptoSession {
    pub fn new(
        ...
        Self {
            ...
            max_recv_nonce: AtomicU64::new(0),
    ...
    ///
    /// Takes `&self` — safe for concurrent use. Replay protection uses
    /// relaxed atomics (approximate window, not cryptographically precise).
    pub fn decrypt(&self, ciphertext: &[u8], out: &mut [u8]) -> Result<usize> {
        ...
        // Basic replay protection: reject very old nonces
        // Allow some slack for reordering (window of 1024 packets)
        let max_seen = self.max_recv_nonce.load(Ordering::Relaxed);
        if nonce + 1024 < max_seen {
            anyhow::bail!("Nonce too old (replay protection)");
        }
        ...
        // Update max seen nonce (relaxed: approximate tracking is fine)
        if nonce > max_seen {
            self.max_recv_nonce.store(nonce, Ordering::Relaxed);
        }
